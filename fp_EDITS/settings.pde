class Parser
{
  ...
  void resultsOut(String _time, String _score)
  {
    XML newT = settings.addChild("time");
    newT.setContent(_time);
    XML newS = settings.addChild("score");
    newS.setContent(_score);
    saveXML(settings, "output.xml");
  } 
}